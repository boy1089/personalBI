...
def main(date):
    ...
    store = file.Storage(r'token-for-google.json')
    creds = store.get()
    ...
    service = build("keep", "v1", http=creds.authorize(Http()) ,static_discovery=False)
    ...
    # response_search = service.mediaItems().search(body=request_body).execute()
    response_search = service.notes().list().execute()
    response_search = service.notes().get("원하는 것")
